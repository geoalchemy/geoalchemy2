...
_FUNCTIONS: List[Tuple[str, Optional[type], Union[None, str, Tuple[str, str]]]] = [
    ...
    # New in PostGIS 3.6 relative to this module's original hand-curated list, added
    # via live pg_proc introspection (see generate_function_overloads.py). Functions with
    # no docstring below have no dedicated documentation anywhere on postgis.net - they
    # are undocumented legacy/compatibility aliases still present in pg_proc.
    ('ST_3DConvexHull', types.Geometry,
     '''Computes the 3D convex hull of a geometry.'''),
    ('ST_AlphaShape', types.Geometry,
     '''Computes an Alpha-shape enclosing a geometry.'''),
    ('ST_ApproxCount', Integer,
     ('''Not documented by PostGIS.''', 'RT_ST_ApproxCount')),
    ('ST_ApproxHistogram', None,
     ('''Not documented by PostGIS.''', 'RT_ST_ApproxHistogram')),
    ('ST_ApproxQuantile', None,
     ('''Not documented by PostGIS.''', 'RT_ST_ApproxQuantile')),
    ('ST_ApproxSummaryStats', types.SummaryStats,
     ('''Not documented by PostGIS.''', 'RT_ST_ApproxSummaryStats')),
    ('ST_Area2D', Float,
     '''Not documented by PostGIS.'''),
    ('ST_AsFlatGeobuf', LargeBinary,
    ...
    ('ST_InterpolateRaster', types.Raster,
     ('''Interpolates a gridded surface based on an input set of 3-d points, using the X- and Y-values to position the points on the grid and the Z-value of the points as the surface elevation.''', 'RT_ST_InterpolateRaster')),
    ('ST_IntersectionFractions', types.Raster,
     ('''Calculates the fraction of each raster cell that is covered by a given geometry.''', 'RT_ST_IntersectionFractions')),
    ('ST_InverseTransformPipeline', types.Geometry,
    ...
    ('ST_IsCoverageTile', Boolean,
     ('''Not documented by PostGIS.''', 'RT_ST_IsCoverageTile')),
    ('ST_LargestEmptyCircle', None,
     '''Computes the largest circle not overlapping a geometry.'''),
    ('ST_Length2DSpheroid', Float,
     '''Not documented by PostGIS.'''),
    ('ST_Letters', types.Geometry,
    ...
    ('ST_LineExtend', types.Geometry,
     '''Returns a line extended forwards and backwards by specified distances.'''),
    ('ST_MaximumInscribedCircle', None,
     '''Computes the largest circle contained within a geometry.'''),
    ('ST_MemCollect', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MLineFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MPointFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MPolyFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiLineFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiLineStringFromText', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiPointFromText', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiPointFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiPolyFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_MultiPolygonFromText', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_NumCurves', Integer,
     '''Return the number of component curves in a CompoundCurve.'''),
    ('ST_OptimalAlphaShape', types.Geometry,
     '''Computes an Alpha-shape enclosing a geometry using an "optimal" alpha value.'''),
    ('ST_PointM', types.Geometry,
     '''Creates a Point with X, Y, M and SRID values.'''),
    ('ST_PointZ', types.Geometry,
     '''Creates a Point with X, Y, Z and SRID values.'''),
    ('ST_PointZM', types.Geometry,
     '''Creates a Point with X, Y, Z, M and SRID values.'''),
    ('ST_PolyFromText', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_PolyFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_PolygonFromWKB', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_ReclassExact', types.Raster,
     ('''Creates a new raster composed of bands reclassified from original, using a 1:1 mapping from values in the original band to new values in the destination band.''', 'RT_ST_ReclassExact')),
    ('ST_ReducePrecision', types.Geometry,
     '''Returns a valid geometry with points rounded to a grid tolerance.'''),
    ('ST_RemoveIrrelevantPointsForView', types.Geometry,
     '''Removes points that are irrelevant for rendering a specific rectangular view of a geometry.'''),
    ('ST_RemoveSmallParts', types.Geometry,
     '''Removes small parts (polygon rings or linestrings) of a geometry.'''),
    ('ST_Scroll', types.Geometry,
     '''Change start point of a closed LineString.'''),
    ('ST_SetGeoTransform', types.Raster,
     ('''Not documented by PostGIS.''', 'RT_ST_SetGeoTransform')),
    ('ST_SetM', types.Geometry,
     ('''Returns a geometry with the same X/Y coordinates as the input geometry, and values from the raster copied into the M dimension using the requested resample algorithm.''', 'RT_ST_SetM')),
    ('ST_SetZ', types.Geometry,
     ('''Returns a geometry with the same X/Y coordinates as the input geometry, and values from the raster copied into the Z dimension using the requested resample algorithm.''', 'RT_ST_SetZ')),
    ('ST_SimplifyPolygonHull', types.Geometry,
     '''Computes a simplified topology-preserving outer or inner hull of a polygonal geometry.'''),
    ('ST_Square', types.Geometry,
    ...
    ('ST_SymmetricDifference', types.Geometry,
     '''Not documented by PostGIS.'''),
    ('ST_TransformPipeline', types.Geometry,
...
#
# Validated against a live PostGIS 3.6.4 instance's pg_proc catalog via
# `generate_function_overloads.py` (see `geoalchemy2._functions_helpers._diff_function_catalog`).
# Signatures that PostGIS supports but that don't change the return type (e.g. an all-text
# WKT-argument overload that still returns Geometry, same as the default) are omitted here -
# they already fall back correctly to the static default in `_FUNCTIONS`.
